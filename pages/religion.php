...
if(!(isset($_SESSION['religion'])&&$_SESSION['religion'][0]==1)) {header("Location: index.php");}


if(isset($_REQUEST['Edit'])){
$query=" select *,count(id) from religion where name='".$_REQUEST['name']."';";
...
$result= mysqli_query($conn, $query);
 while ($row=mysqli_fetch_assoc($result)){
   if($row["count(id)"]>=1){
    ...
    mysqli_query($conn,"UPDATE `school`.`religion` SET `name`='".$_REQUEST['name']."' WHERE `id`='".$_REQUEST['id']."';");
    }
 ...
 while ($row=mysqli_fetch_assoc($result)){
   if($row["count(id)"]>=1){
    echo"<script>alert('This entry has already been added')</script>";
        ...
        mysqli_query($conn,"INSERT INTO `school`.`religion` (`name`) VALUES ('".$_REQUEST['name']."');");
...
{
    mysqli_query($conn,"DELETE FROM `school`.`religion` WHERE `id`='".$_REQUEST['Delete']."';");
...
<body>
       ...
       <form action="menu.php">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Religion List</h1>
                    <h4>Warning Foreign Keys cannot be deleted</h4>
                </div>

<!--justin lo--> 
                <!--------------------------------------------------------------------------modal------------------------------------------------------------------------------------>
                 <?php if($_SESSION['religion'][1]==1){ echo"&nbsp &nbsp &nbsp &nbsp <button type='button' class='btn btn-info btn-lg' data-toggle='modal' data-target='#myModal' id='insert' onclick='buttonInsert()'>Insert</button>";}?>
                    
                <!-- Trigger the modal with a button -->

        <br/><br/><br/>
        <!-- Modal -->
    <form method='get' action='religion.php'>
                            ...
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Name</th>
                                        <?php if($_SESSION['religion'][2]==1){ echo"<th></th><th></th>";} ?>
                                             ...
                                             $query="SELECT * from religion";
                                                ...
                                                while ($row=mysqli_fetch_assoc($result)){
                                                    ...
                                                    print("
                                                        ...
                                                        <input type=hidden id=id$x value=".$row['id'].">");
                                                         if($_SESSION['religion'][2]==1){ echo"<td style='width:50px;'><center><button type='button' class='btn btn-info btn-lg' data-toggle='modal' data-target='#myModal' onclick='buttonEdit($x)'>Edit</button></center></td>";}
                                                         if($_SESSION['religion'][2]==1){ echo"<form method='get' action='religion.php'>
                                                          <td style='width:50px;'><center><input type='submit' class='btn btn-info btn-lg' value='Delete'></button></center></td>
                                                          <input type='hidden' name='Delete' value=".$row['id']."> </form>";}
